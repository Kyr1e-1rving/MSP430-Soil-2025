...
#define MODULE_ID 100 // change between each flash!
...
int main(void) {
    ...
    while(1) {
        ...
        //Take Oxygen Reading
        init_oxygen();            //Calls read_oxygen(); within itself
        
        // power pins for sensors
        P5DIR |= BIT3; //SERVO_GPIO_EN
        ////P5DIR |= BIT2;
        ////P4DIR |= BIT5;
        P3DIR |= BIT4; //3V3_LORA_EN
        P3DIR |= BIT6; //3V3_SENSORS_EN
        ...
        // turn on all sensors
        ////P5OUT |= BIT2;
        P5OUT |= BIT3; //SERVO_GPIO_EN
        //P3OUT |= BIT4; //3V3_LORA_EN
        P3OUT |= BIT6; //3V3_SENSORS_EN
        ////P4OUT |= BIT5;
        ...
        /*Open Servo;    
        iteration = 0;
        if (iteration == 0) { // only activate the servo if we need to, the disk will give if we turn it on even for a tick
            P5OUT |= BIT3; ////SERVO_GPIO_EN
            init_timer_B_open();
            wireless_buf[34] = 1;
            seesaw_buf[25] = 1;
        ...
        __delay_cycles(100000); // give it some time to send
        wireless_send(o2_buf, 14); // send oxygen data
        __delay_cycles(100000); // give it some time to send        
        P3OUT &= ~BIT4; //3V3_LORA_EN
